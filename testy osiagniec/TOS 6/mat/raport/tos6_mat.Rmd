...
Możemy już wyestymować pierwszy model. Do tego celu wykorzystamy funkcję `tam.mml.mfr()`.
...
fit0 = tam.fit(mod0, progress = FALSE)
# scalenie zbiorów danych
mod0_fit = cbind(mod0$xsi, fit0)

# niedopasowane zadania <0,8 lub >1,2
out0 = rownames(mod0_fit)[(mod0_fit[, 3] < 0.8) | (mod0_fit[, 3] > 1.2)]
in0 = rownames(mod0_fit)[(mod0_fit[, 7] < 0.8) | (mod0_fit[, 7] > 1.2)]
round(mod0_fit[, c(1:5, 7:9)], 3)
```
...
mod0_md = melt(data.frame(item = dimnames(mod0_fit)[[1]], mod0_fit), "item")
...
Przyjrzyjmy się im bliżej. Poniżej znajdują się wykresy z krzywą dla przewidywanego wyniku dla tych zadań z naniesioną krzywą empiryczną.

```{r, results = "hide", echo = FALSE, fig.cap = c("Krzywa przewidywanego wyniku w zależności od poziomu umiejętności dla zad.X")}
...
#fig.cap = c(paste("Krzywe empiryczne dla kategorii zadania", out0[1]), paste("Krzywe empiryczne dla kategorii zadania", out0[2]))
...
plotctt(mat_all[, out0, drop = FALSE], theta = mod0_pv[, 2], Ncuts = 8)

```

Dla zadania `r out0[1]` poprawną odpowiedzią jest odpowiedź C (kod 3), natomiast dla zadania `r out0[2]` - B (kod 2). Theta (estymowana za pomocą PV) została podzielona na 8 równolicznych przedziałów. Dla tak określonych grup wyników wyliczone zostały odsetki uczniów, którzy otrzymali poszczególne kody w zadaniu. Dla obu zadań można wskazać dystraktor, który ma dużej części niemalejące prawdopodobieństwo wyboru (dopiero w ostatniej grupie wyników zaczyna spadać). I tak, dla zadania `r out0[1]` taką niewzruszoną konkurencję dla poprawnej odpowiedzi stanowi odpowiedź A (kod 1), natomiast dla zadania `r out0[2]` to odpowiedź C (kod 3). Na tym etapie analiz postanowiono jednak nie usuwać tych zadań.
...
plot(mod0, grep("KMA_10|KMA_22", mod0$item$item),
...
Sytuację te można także zobrazować za pomocą tzw. progów Thurstone'a (*Thurstonian tresholds*). Progi te odpowiadają pozycji na skali umiejętności, dla których wartość funkcji prawdopodobieństwa uzyskania danej kategorii lub wyższej wynosi 50%. W przypadku zadania ocenianego na skali 0-1-2 analizowane są dwie krzywe: jedna przedstawiąjąca zależność pomiędzy umiejętnością a prawdopodobieństwem uzyskania kategorii 1 lub 2, a druga - uzyskania kategorii 2.

```{r, echo = FALSE, results = "hide"}
# tresholdy dla mod0
mod0_tt = tam.threshold(mod0)
head(mod0_tt)
...
W teście matematycznym znajduje się `r nrow(kotw)` zadań kotwiczących. Poprzednie skalowanie bazowało na założeniu, że zadania te funkcjonują tak samo w obu wersjach testu. W tej części raportu założenie to zostanie sprawdzone.
