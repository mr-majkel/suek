...
```{r, echo = FALSE}
# wyliczenie miar dopasowania
fit2 = tam.fit(mod2, progress = FALSE)
# scalenie zbiorów danych
mod2_fit = cbind(mod2$xsi, fit2)

# niedopasowane zadania <0,8 lub >1,2
out2 = rownames(mod2_fit)[(mod2_fit[, 3] < 0.8) | (mod2_fit[, 3] > 1.2)]
in2 = rownames(mod2_fit)[(mod2_fit[, 7] < 0.8) | (mod2_fit[, 7] > 1.2)]
round(mod2_fit[, c(1:5, 7:9)], 3)
```

```{r, echo = FALSE, fig.cap= "Wykres skrzynkowy dla miar dopasowania zadań w teście matematycznym TOS6"}
# wykres pudełkowy dla miar dopasowania
mod2_md = melt(data.frame(item = dimnames(mod2_fit)[[1]], mod2_fit), "item")
...
Analizie poddane będą jednak tylko zadania kotwiczące i oszacowane różnice w trudności ze względu na wersję.

```{r}
mod3$xsi[grep("^K.*A$|KMA_10:wersjaA:step1|KMA_22:wersjaA:step1",
              rownames(mod3$xsi)), ]
```


# Podsumowanie uwag do dalszych zastsowań testu

1. Usunąć zad. MB_25 (nietrafne).
